...
$results = $con->query("SELECT * FROM media WHERE relation = 'page' AND relation_id = '".$_GET['id']."'
    ORDER BY date DESC, time DESC LIMIT $position, $items_per_group")or die($mysqli->error) ?>
    <ul class="page_result">
	<?php while($result = $results->fetch_object()): ?>
       <li id="item_<?= $result->id ?>">
       
       
       <?php if($result->author_id == "page"):?>

 	<?php $user_loadd = $con->query("SELECT * FROM pages WHERE id = ".$_GET['id']."")or die($mysqli->error);?>
	<?php while($user_loadedd = $user_loadd->fetch_object()): ?>
	<?php $name = $user_loadedd->name; ?>
 	<?php endwhile;?>
	

       <?php else:?>
 	<?php $user_load = $con->query("SELECT * FROM users WHERE id = ".$result->userID."")or die($mysqli->error);?>
	<?php while($user_loaded = $user_load->fetch_object()): ?>
	<?php $name = $user_loaded->firstname.$user_loaded->lastname; ?>
	<?php endwhile;?>
				...
				<div id="Feed_Contents_Loop" class="Feed_Contents_Loop">
					<div class="Header_Feed_Contents_Loop Video">
						<div class="row">
							...
							<div class="col-xs-4">
								<h4 style="margin-bottom:2px;padding-bottom:0px;"><?= $name ?> <?= $result->shared ?> Posted at <?= $result->date ?> <?= $result->time ?></h4>
							</div>
							<div class="col-xs-2">
